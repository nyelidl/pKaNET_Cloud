...
# Sulfonamide secondary N-H (H1) bonded to any aromatic ring atom.
# The Python-side helper _sulfonamide_hetn_nidx() confirms the ring contains
# an aromatic N, which lowers pKa to ~5.5 (thiazole, thiadiazole, pyrimidine, etc.).
_PAT_SULFONAMIDE_HETN_NH         = Chem.MolFromSmarts("[NX3;H1]([SX4](=O)(=O))[a]")
...
def _sulfonamide_hetn_nidx(mol, n_idx):
    """
    Return True if the sulfonamide N at n_idx is bonded to an aromatic ring
    that contains at least one aromatic nitrogen (n).
    This distinguishes heteroaryl-sulfonamides (pKa ~5-6, e.g. sulfamethizole,
    sulfadiazine, sulfathiazole) from plain aryl-sulfonamides (pKa ~9.7).
    """
    ...
    for nb in n_atom.GetNeighbors():
        if not nb.GetIsAromatic():
            continue
        # Find which rings this aromatic neighbour belongs to
        for ring in ri.AtomRings():
            if nb.GetIdx() not in ring:
                continue
            # Check ring contains an aromatic N
            for idx in ring:
...
def find_ionizable_sites(mol):
    ...
    # (16) Heteroaryl sulfonamide N-H: sulfonamide NH directly bonded to an
    #      N-containing heteroaromatic ring (thiazole, thiadiazole, pyrimidine,
    #      pyridine, triazole, …).  The ring N is strongly EWG, lowering pKa
    #      to ~5-6 vs. ~9.7 for plain aryl sulfonamides.
    #      Literature: sulfamethizole pKa 5.45, sulfadiazine 6.4, sulfathiazole 7.1
    #      We use a conservative mid-range value of 5.7 so that at pH 7.4 the
    #      deprotonated (charge -1) state is correctly ranked as dominant.
    if _PAT_SULFONAMIDE_HETN_NH is not None:
        for match in mol.GetSubstructMatches(_PAT_SULFONAMIDE_HETN_NH):
            nidx = match[0]
            if nidx in seen_ion or nidx in claimed_atoms:
                continue
            if not _sulfonamide_hetn_nidx(mol, nidx):
                continue  # ring has no aromatic N → keep for generic table
            seen_ion.add(nidx); claimed_atoms.add(nidx)
            sites.append(dict(label="sulfonamide_hetaryl_NH",
                              atom_indices=[nidx],
                              heuristic_pka=5.7,
                              site_type="acid"))
